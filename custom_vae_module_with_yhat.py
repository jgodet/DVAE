...
# isn't used anymore.   custom_vae_module.py is better
class ModularVAEWithYHat(nn.Module):
    def __init__(self, input_dim=784, hidden_dim=4096, latent_dim=100, num_labels=10, use_labels_in_input = False, debug_mode=False):
        ...
        super(ModularVAEWithYHat, self).__init__()
        ...
        self.fc2_yhat = nn.Linear(hidden_dim, num_labels) 
        self.fc3 = nn.Linear(latent_dim + num_labels, hidden_dim)
        self.fc4 = nn.Linear(hidden_dim, input_dim) 

    def encode(self, x):  # currently unused
        h = F.relu(self.fc1(x))
        return self.fc2_mean(h), self.fc2_logvar(h), F.softmax(self.fc2_yhat(h), dim=1)

    def reparameterize(self, mu, logvar): # to calculate z
    ...
    def predict_labels(self, x):
        h = F.relu(self.fc1(x))
        return F.softmax(self.fc2_yhat(h), dim=1)
...
def train_modular_vae_with_yhat(model, train_loader, optimizer, epoch, beta, device):
    model.train()
    train_loss = 0
    correct = 0
